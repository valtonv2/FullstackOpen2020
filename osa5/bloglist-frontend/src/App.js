...
const App = () => {
  ...
  const updateBlog = async (blog) => {
    try{
      ...
      setBlogs(blogs.filter(b => b.id !== blog.id).concat(updateResult).sort(blogCompare))
      setAnnouncement(`Blog ${updateResult.title} updated succesfully`)
      setTimeout(() => {setAnnouncement(null)}, 5000)
  ...
  const deleteBlog = async (blog) => {
    try{
      ...
      if(window.confirm(`Delete blog ${blog.title} ?`)){
        ...
        setAnnouncement(`Blog ${blog.title} deleted succesfully`)
        setTimeout(() => {setAnnouncement(null)}, 5000)
